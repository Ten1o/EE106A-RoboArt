...
parse.add_argument('--root', type=str, default='..', help='path to data set')
...
parse.add_argument('--restore', type=int, default=1, help='whether to restore checkpoints')
...
parse.add_argument('--num_class', type=int, default=20, choices=range(10, 3755))
...
class NetSmall(nn.Module):
    ...
    # def forward(self, x):
    #     # print(x.shape) torch.Size([1, 1, 64, 64])
    #     x = self.pool(F.relu(self.conv1(x)))
    #     # print(x.shape) torch.Size([1, 6, 31, 31])
    #     x = self.pool(F.relu(self.conv2(x)))
    #     # print(x.shape) torch.Size([1, 16, 13, 13])
    #     x = x.view(-1, self.num_flat_features(x))
    #     # print(x.shape) torch.Size([1, 2704])
    #     x = F.relu(self.fc1(x))
    #     x = F.relu(self.fc2(x))
    #     x = self.fc3(x)
    #     return x
    def __init__(self):
        ...
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(6, 16, 5)
        self.conv3 = nn.Conv2d(16, 120, 13)
        self.fc1 = nn.Linear(120, 84)
        self.fc2 = nn.Linear(84, args.num_class)
...
def inference():
    ...
    if 'Chinese_character_recognition_final' in sys.path:
        args.log_path = 'Chinese_character_recognition_final/log.pth'
    checkpoint = torch.load(args.log_path)
    model.load_state_dict(checkpoint['model_state_dict'])
    output = model(input)
